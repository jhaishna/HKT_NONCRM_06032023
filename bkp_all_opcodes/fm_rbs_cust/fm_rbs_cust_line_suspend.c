#ifndef lint
static const char Sccs_id[] = "@(#)%Portal Version: fm_rbs_cust_line_suspend.c :BillingVelocityInt:3:2006-Sep-05 21:55:03 %";
#endif
    ...
    pin_errbuf_t    *ebufp)
{
        ...
        *r_flistpp              = NULL;
         ...
         * Insanity Check
         *******************************************************************/
        if (opcode != PCM_OP_RBS_CUST_SUSPEND_LINE ) {
        ...
        *******************************************************************/
        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                "op_rbs_cust_line_suspend input flist", i_flistp);

        /*******************************************************************
         * Call the default implementation
         *******************************************************************/

        fm_rbs_cust_line_suspend(ctxp, i_flistp, &r_flistp, ebufp);
	...
	if (PIN_ERRBUF_IS_ERR(ebufp))
	{
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"op_rbs_cust_line_suspend error", ebufp);
	}
	else
	{
		*r_flistpp = PIN_FLIST_COPY(r_flistp, ebufp);
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "op_rbs_cust_line_suspend output flist", *r_flistpp);
	}
        ...
        pin_errbuf_t            *ebufp)
{
	...
	char                    *status_msg = NULL;
        ...
        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "start fm_rbs_cust_line_suspend input_flist", i_flistp);
	...
	if (pur_pdct)
        ...
        if (pur_pdct && p_status && p_descr && *p_status == 1 && strstr(p_descr, "SUS:I;"))
        {
		...
		PIN_FLIST_DESTROY_EX(&read_flistp, ebufp);
        ...
        if (pur_dsct && p_status && p_descr && *p_status == 1 && strstr(p_descr, "SUS:I;"))
        {
		...
		if (PIN_ERR_IS_ERR(ebufp))
                {
                	PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "PCM_OP_SUBSCRIPTION_SET_DISCOUNT_STATUS error!", ebufp);
                        goto CLEANUP;
                }
                PIN_ERR_LOG_FLIST(3, "discount status output", ret_disc_flistp);
        }
	
	PIN_ERR_LOG_POID(3, "account_poid: ", acc_pd);
	PIN_ERR_LOG_POID(3, "service_poid: ", ser_obj);

	if (acc_pd && ser_obj && p_status && p_descr && *p_status == 1 && strstr(p_descr, "CYC:CA"))
        {
                ...
                PIN_FLIST_ELEM_COPY(search_oflistp, PIN_FLD_RESULTS, 0, ar_in_flistp, PIN_FLD_PRODUCTS, 0, ebufp);
		
	//	PIN_ERR_LOG_FLIST(3, "ar_in_flistp: ", ar_in_flistp);
		
              //  if (p_status && p_descr && *p_status == 1 && strstr(p_descr, "CYC:CA"))
              //  {
	...
	if (type_str && strcmp(type_str, "RT") == 0)
	{
		...
		PIN_ERR_LOG_MSG(3, msg);	
	        if (p_status && p_descr && *p_status == 1 && strstr(p_descr, "SUS:R;"))
		{
			PIN_ERR_LOG_FLIST(3, "PCM_OP_MRB_POL_REBATE_SUSPENSE input: ", inp_flistp);
			PCM_OP(ctxp, PCM_OP_MRB_POL_REBATE_SUSPENSE, 0, inp_flistp, &ret_flistp, ebufp);
			if (PIN_ERR_IS_ERR(ebufp))
        		{
                        	PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "PCM_OP_MRB_POL_REBATE_SUSPENSE error!", ebufp);
                        	goto CLEANUP;
                	}
	
			PIN_ERR_LOG_FLIST(3, "PCM_OP_MRB_POL_REBATE_SUSPENSE output: ", ret_flistp);	
		}
                if (p_status && p_descr && *p_status == 1 && strstr(p_descr, "SUS:O;"))
		{
			PIN_ERR_LOG_FLIST(3, "PCM_OP_MRB_POL_OINCENTIVE_SUSPENSE input", inp_flistp);
			PCM_OP(ctxp, PCM_OP_MRB_POL_OINCENTIVE_SUSPENSE, 0, inp_flistp, &ret_flistp, ebufp);
			if (PIN_ERR_IS_ERR(ebufp))
			{
                		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "PCM_OP_MRB_POL_OINCENTIVE_SUSPENSE error!", ebufp);
                	        goto CLEANUP;
                	}
			PIN_ERR_LOG_FLIST(3, "PCM_OP_MRB_POL_OINCENTIVE_SUSPENSE output", ret_flistp);
		}
		if (p_status && p_descr && *p_status == 1 && strstr(p_descr, "SUS:PTC;"))
                {
        		PIN_ERR_LOG_FLIST(3, "PCM_OP_MRB_POL_SUSPEND_CONTRACT input", inp_flistp);
        		PCM_OP(ctxp, PCM_OP_MRB_POL_SUSPEND_CONTRACT, 0, inp_flistp, &ret_flistp, ebufp);
			if (PIN_ERR_IS_ERR(ebufp))
                	{
                	        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "PCM_OP_MRB_POL_OINCENTIVE_SUSPENSE error!", ebufp);
                		goto CLEANUP;
			}
                	PIN_ERR_LOG_FLIST(3, "PCM_OP_MRB_POL_SUSPEND_CONTRACT output", ret_flistp);
       		}
                if (p_status && p_descr && *p_status == 1 && strstr(p_descr, "SUS:ITC;"))
		{ 
        		PIN_ERR_LOG_FLIST(3, "PCM_OP_MRB_POL_SUSPEND_INSTALLMENT input", inp_flistp);
                	PCM_OP(ctxp, PCM_OP_MRB_POL_SUSPEND_INSTALLMENT, 0, inp_flistp, &ret_flistp, ebufp);
               		if (PIN_ERR_IS_ERR(ebufp))
                	{
                	        PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR, "PCM_OP_MRB_POL_SUSPEND_INSTALLMENT error!", ebufp);
                		goto CLEANUP;
			}
                	PIN_ERR_LOG_FLIST(3, "PCM_OP_MRB_POL_SUSPEND_INSTALLMENT output", ret_flistp);
        	}	
	}
	...
	PIN_ERR_LOG_FLIST(3, "PCM_OP_CUST_MODIFY_SERVICE output:", mod_o_flistp);

	if (PIN_ERRBUF_IS_ERR(ebufp))
       	{
		ret_flistp = PIN_FLIST_CREATE(ebufp);
		s_fail = 1;
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,"fm_rbs_cust_line_suspend error", ebufp);
                ...
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "error fm_rbs_cust_line_suspend flist", ret_flistp);
		...
		*r_flistp = PIN_FLIST_COPY(ret_flistp, ebufp);
		...
		ret_flistp = PIN_FLIST_CREATE(ebufp);
                ...
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG, "fm_rbs_cust_line_suspend output flist", ret_flistp);
		*r_flistp = PIN_FLIST_COPY(ret_flistp, ebufp);
        }
	return;
CLEANUP:
	*r_flistp = PIN_FLIST_COPY(i_flistp, ebufp);
	...
	PIN_FLIST_DESTROY_EX(&mod_o_flistp, NULL);
	PIN_FLIST_DESTROY_EX(&ret_flistp, NULL);
	PIN_FLIST_DESTROY_EX(&ret_disc_flistp, NULL);
	PIN_FLIST_DESTROY_EX(&ar_o_flistp, NULL);
